...
export default function Profile({ data, onSave }) {
    ...
    const handlePresentationChange = useCallback((bookId, field, value) => {
        setEditableProfile(prev => ({
            ...prev,
            Present: prev.Present.map(p => p.bookId === bookId ? { ...p, [field]: value } : p)
        }));
    }, []);
    ...
    if (!editableProfile.Skill) return <div>Đang tải hồ sơ...</div>;
            ...
            <div className={styles.profileBody}>
                <div className={styles.card}>
                    ...
                    <div className={styles.cardContent}>
                        <div className={styles.textareaWrapper}><textarea className={`input text_4_m ${styles.textarea}`} value={Intro || ''} onChange={handleIntroChange} /></div>
