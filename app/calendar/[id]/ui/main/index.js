            ...
            {showImagePopup && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0, 0, 0, 0.7)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1050, cursor: 'pointer' }} onClick={() => setShowImagePopup(false)}>
                    <div style={{ position: 'relative', width: '60vh', height: '60vh', backgroundColor: 'white', overflow: 'hidden', borderRadius: '8px' }} onClick={(e) => e.stopPropagation()}>
                        <Image src={popupImageUrl} alt="Student Avatar Popup" fill style={{ objectFit: 'contain' }} />
                    </div>
                </div>
            )}
                        ...
                        <section className={styles.infoSection}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                                ...
                                <div style={{ display: 'flex', gap: 8 }}>
                                    <button onClick={reloadData} disabled={saving} className="text_6_400" style={{ padding: '8px 16px', background: saving ? 'var(--text-disabled)' : 'var(--green)', color: '#fff', border: 'none', borderRadius: 5, cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                                        {saving && <span className={styles.spinner} />}
                                        {saving ? 'Đang tải lại…' : 'Tải lại dữ liệu '}
                                    </button>
                                    <button onClick={saveAll} disabled={saving} className="text_6_400" style={{ padding: '8px 16px', background: saving ? 'var(--text-disabled)' : 'var(--green)', color: '#fff', border: 'none', borderRadius: 5, cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                                        {saving && <span className={styles.spinner} />}
                            ...
                            {roll.length ? (
                                <>
                                    <div className={`${styles.row} ${styles.headerRow}`}>
                                        {headers.map((t, i) => (
                                            <div key={t} className="text_6_400" style={{ flex: t === 'Học sinh' ? 3 : (t === 'Có mặt' || t === 'Vắng mặt') ? (isTrialCourse ? 1.5 : 1) : 1, color: '#fff', padding: i <= 1 ? 8 : '8px 0', textAlign: i <= 1 ? 'left' : 'center' }}>
                                                {t}
                                            </div>
                                        ))}
                                        {course.Version != 0 && (<div className="text_6_400" style={{ flex: 1, color: '#fff', padding: '8px 0', textAlign: 'center' }}>Hình ảnh</div>)}
                                    </div>
                                    {roll.map(stu => {
