        ...
        <div className={styles.wrapper}>
            ...
            {!currentType && (
                <div className={styles.filterRow}>
                    ...
                    <Menu isOpen={isLabelMenuOpen} onOpenChange={setIsLabelMenuOpen} customButton={<div className='input text_6_400'>{getSelectedName('label', labels, 'Tất cả nhãn', '_id', 'title')}</div>} menuItems={<div className={styles.menulist}><p className='text_6_400' onClick={() => { createURL({ label: '' }); setIsLabelMenuOpen(false); }}>Tất cả nhãn</p>{labels.map(l => <p key={l._id} className='text_6_400' onClick={() => { createURL({ label: l._id }); setIsLabelMenuOpen(false); }}>{l.title}</p>)}</div>} menuPosition="bottom" />
