...
export default function SendCmt({ data, lesson }) {
    ...
    const filteredStudents = useMemo(() => {
        return data?.Student.filter(student => student.Learn?.some(item => item.Lesson === lesson)) ?? [];
    }, [data, lesson]);
    return (
        <>
            <div className='btn' style={{ marginTop: 8, borderRadius: 5, background: 'var(--main_d)', cursor: 'pointer' }} onClick={() => setIsPopupOpen(true)}>
                ...
                renderItemList={students => students.length > 0 ?
                    (<ul style={{ listStyle: 'none', padding: '0 15px', margin: 0 }}>
                        {students.map(student => (
                            <li key={student._id} style={{ padding: '10px 0', borderBottom: '1px solid #eee' }}>
                                {student.Name}
                            </li>
                        ))}
                    </ul>) :
                    (<p style={{ textAlign: 'center', padding: '20px' }}>Không có học sinh nào phù hợp.</p>)
                }
            />
        </>
    );
